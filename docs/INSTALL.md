...
## 4. 从当前任务打开紧凑版 Dashboard

插件 MCP 启动时会一并启动本机 Dashboard 服务。在当前 Codex 任务的聊天框输入 `@` 并选择 **Thread Shelf**；也可以输入 `/`，在 **Skills** 分组中找到并选择 Thread Shelf。直接发送 mention 即可，不需要补充自然语言。插件会绑定当前任务并返回：

[http://127.0.0.1:43125/?compact=1](http://127.0.0.1:43125/?compact=1)

点击链接即可在当前任务的 Codex 内置浏览器打开；可用的 Browser 能力也可以直接导航到该地址。切换到另一个任务时，在新任务的聊天框再次选择 `@Thread Shelf`，页面就会绑定并显示那个任务的收藏。普通宽版仍可通过 `http://127.0.0.1:43125/` 使用。

`/` 入口是从 Skills 分组选择技能，并不是固定的 `/thread-shelf` 自定义命令。聊天框的 `+` 是文件、图片等附件入口，不是插件选择器。

Codex 内置浏览器按任务隔离，正好让每个任务保留自己的浏览上下文。`Cmd/Ctrl+Shift+B` 显示或隐藏当前任务的内置浏览器区域。只有明确需要一个标签页跨任务常驻时，才点击页面中的“跨任务固定到系统浏览器”，并在 Chrome、Edge 或 Safari 固定它。
...
1. 在任务 A 输入 `@` 选择 Thread Shelf，并从返回链接在 Codex 内置浏览器打开。
2. 在任务 A 选择一个文件并收藏，再从候选区点星标收藏一个网址或输出文件。
3. 打开并定位收藏，确认系统动作正确。
4. 移除收藏，确认原文件仍然存在。
5. 切换到任务 B，再次选择 `@Thread Shelf` 并打开，确认顶部当前任务已切换。
...
- 不能向 Codex 原生右键菜单、内置浏览器工具栏、标题栏或右侧栏加入按钮。
- Dashboard 是独立浏览器页；紧凑版不会覆盖、修改或真正固定到 Codex 客户端 UI。
- Codex 内置浏览器标签页按任务隔离；切换任务后，在新任务中再次用 `@` 或 `/` 选择 Thread Shelf。普通浏览器固定标签页只是可选的跨任务常驻方式。
