...
推荐在 Chrome、Edge 或 Safari 中只固定这一个标签页。它会读取 Hook 更新的当前任务，切换 Codex 任务时无需重开网址。普通宽版仍可通过 `http://127.0.0.1:43125/` 使用。

如果页面当前位于 Codex 内置浏览器，点击提示条中的“在系统浏览器打开”，然后在普通浏览器中固定新标签页。

Codex 内置浏览器按任务隔离，适合在单个任务中临时放在右侧查看，但其标签页不会跨任务常驻。`Cmd/Ctrl+Shift+B` 只显示或隐藏当前任务的内置浏览器区域，不能把插件注册成宿主原生侧栏。
...
1. 在普通浏览器固定一个紧凑版 Dashboard 标签页，在任务 A 选择一个文件并收藏。
...
- Codex 内置浏览器标签页按任务隔离；普通浏览器中的单个固定标签页才是跨任务免重开的可靠入口。
